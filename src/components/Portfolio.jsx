...
function Portfolio(props) {
    ...
    console.log(data.portfolio_url)
  ...
  return (
    <section className='flex justify-center my-5 px-5'>
        <div style={{backgroundImage: `url(${data.img})`, backgroundPosition: `${data.title === "NETFLIX CLONE" && "top"}`}} className={`w-[60rem] sm:w-[60rem] md:w-[60rem] lg:w-[110rem]  h-[33.9rem] rounded-[50px] bg-no-repeat bg-center bg-cover transition ease-linear`}>
            <div className=' flex flex-row-reverse pr-5 bg-black/80 hover:bg-black/60 transition-all ease-in-out cursor-pointer duration-500 rounded-[50px]'>
                <div className='flex flex-col justify-center h-[542px] items-end pr-10 xsm:pr-0 xxsm:pr-0'>
                    ...
                    <span className='mt-4'><a target='_blank' href={`${data.id === data.id ? data.portfolio_url : ""}`} className='border-solid border-2 p-3 border-white hover:bg-white text-white font-thin text-[12px] hover:text-black transition ease-linear'>{data.role === "FRONTEND DEVELOPER" ? "GITHUB" : "BEHANCE"}</a></span>
                </div>
            </div>
        </div>
    </section>
